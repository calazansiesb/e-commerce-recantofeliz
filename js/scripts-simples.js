...
// Produtos para desenvolvimento local
const produtosLocal = [
    {
        id: 1,
    ...
    },
    {
        ...
        price: 15,
        image: "imagens/produtos/3.1.jpg",
        stock: 120,
        active: true
    },
    {
        ...
        price: 25,
        image: "imagens/produtos/4.1.jpg",
        stock: 80,
        active: true
    },
    {
        ...
        price: 34,
        image: "imagens/produtos/5.1.jpg",
        stock: 50,
        active: true
    },
    {
        ...
        price: 45,
        image: "imagens/produtos/6.1.jpg",
        stock: 15,
        active: true
    },
    {
        ...
        price: 40,
        image: "imagens/produtos/7.1.jpg",
        stock: 8,
        active: true
    },
    {
        ...
        name: "Mel",
        category: "mel",
        slogan: "Doçura artesanal. O sabor do campo em cada gota.",
        description: "Mel puro. O toque da natureza nas suas mãos.",
        price: 50,
        image: "imagens/produtos/8.1.jpg",
        stock: 10,
        active: true,
        partner: {
            producer: "Apiário Flor do Cerrado",
            location: "Sobradinho - DF",
            description: "Produção artesanal de mel puro há mais de 10 anos"
        }
    },
    {
        id: 9,
        ...
        price: 37,
        image: "imagens/produtos/9.1.jpg",
        stock: 20,
        active: true,
        partner: {
    ...
    },
    {
        ...
        description: "Café Gourmet Artesanal é uma experiência única. Cultivado com cuidado e colhido à mão, cada grão é torrado lentamente para realçar seu sabor complexo.",
        price: 139,
...
// Carregar produtos
// Carregar produtos diretamente da API DynamoDB
async function carregarProdutos() {
    try {
        ...
        console.error('❌ Erro ao carregar do DynamoDB:', error);
...
function renderizarProdutos() {
    ...
    if (produtos.length === 0) {
        grid.innerHTML = `
            <div class="col-span-full text-center py-12">
                <h3 class="text-xl font-bold text-gray-600 mb-4">Carregando produtos...</h3>
    ...
    grid.innerHTML = produtos.map(produto => `
        <div class="product-card bg-white rounded-lg shadow-lg p-6 flex flex-col items-center text-center transition-all duration-300 hover:shadow-xl relative">
            ...
            <img src="${produto.image}" alt="${produto.name}" class="w-40 h-40 object-cover rounded-lg mb-4" onerror="this.src='imagens/produtos/default/placeholder.jpg'">
...
window.filterProducts = function(category) {
    ...
    if (category === 'granja') {
        ...
        filtered = produtos.filter(p => p.partner || ['mel', 'laticinios', 'bebidas'].includes(p.category));
...
// Função para descobrir imagens do produto (OTIMIZADA - sem delay)
// ✅ PADRÃO: Usar APENAS .jpg para todas as imagens de produtos
function discoverProductImages(productId) {
    // ⚡ Versão síncrona e instantânea - não aguarda descoberta
    // As imagens são definidas no objeto do produto (dados/produtos.json)
    const product = produtos.find(p => p.id == productId);
    if (!product || !product.gallery || product.gallery.length === 0) {
        // Fallback: usar imagem principal do produto
...
function openProductModal(productId) {
    ...
    // 🚀 Instantâneo - não aguarda descoberta de imagens
    const gallery = discoverProductImages(productId);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    setTimeout(() => {
        if (produtos.length < 10) {
            console.log('Retry 1: carregando produtos novamente...');
            carregarProdutos();
        }
    }, 3000);
    ...
    setTimeout(() => {
        if (produtos.length < 10) {
            console.log('Retry 2: carregando produtos novamente...');
            carregarProdutos();
        }
    }, 5000);
