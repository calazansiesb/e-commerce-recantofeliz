...
async function carregarProdutos() {
    try {
        ...
        const apiBase = isDev 
            ? 'https://frb45jmipc.execute-api.sa-east-1.amazonaws.com/dev'
            : 'https://frb45jmipc.execute-api.sa-east-1.amazonaws.com/prod';
            
        const response = await fetch(`${apiBase}/api/produtos`, {
            cache: 'no-store'
        });
...
async function excluirProduto(id) {
    if (confirm('Tem certeza que deseja excluir este produto?')) {
        try {
            // Detectar ambiente para usar a API correta
            const isDev = window.location.pathname.startsWith('/dev/');
            const apiBase = isDev 
                ? 'https://frb45jmipc.execute-api.sa-east-1.amazonaws.com/dev'
                : 'https://frb45jmipc.execute-api.sa-east-1.amazonaws.com/prod';

            const response = await fetch(`${apiBase}/api/produtos/${id}`, {
                method: 'DELETE'
            });
            
            if (response.ok) {
                alert(`Produto excluído do ambiente ${isDev ? 'DE DESENVOLVIMENTO' : 'DE PRODUÇÃO'}!`);
                await carregarProdutos();
...
async function salvarProduto(event) {
    ...
    try {
        ...
        if (window.currentProductImages && window.currentProductImages.length > 0) {
            try {
                // Gerar ID único para novos produtos usando timestamp para evitar colisões
